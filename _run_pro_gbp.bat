...
echo.
echo ============================================================
echo   MT5 PRO BOT  ^|  GBPUSD  ^|  H1 trend + M15 signal
...
echo   Lot: 0.5  ^|  Max spread: 1.8 pips
echo ============================================================
echo.

python -m mt5_bot check --config config/pro_gbp.yaml
if errorlevel 1 (
...
echo.
echo [CHECK OK] Starting PRO GBP loop...
echo Ctrl+C to stop.
echo.

python -m mt5_bot trade ^
    --config config/pro_gbp.yaml ^
    --db data/pro_gbp.sqlite ^
    --trade-enabled

echo.
echo ============================================================
echo   PRO GBP LOOP ENDED — generating report
echo ============================================================
echo.

python -m mt5_bot report --db data/pro_gbp.sqlite
